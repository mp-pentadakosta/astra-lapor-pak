...
export const DashboardUserViewModel = () => {
    const [loading, setLoading] = useState(false);

    const [dashboard, setDashboard] = useState<ModelDashboardAdmin>();

    const getTotalPengajuan = async () => {
        const resp = await DashboardAdminRepository("user", EnumTipeDashboard.totalPengajuan)
        if (resp !== null) {
            setDashboard({
                ...dashboard,
    ...
    const getPengajuanDitinjau = async () => {
        const resp = await DashboardAdminRepository("user", EnumTipeDashboard.totalDitijau)
        if (resp !== null) {
            // setDashboard((prevState) => {
            //     return {
            //         ...prevState,
    ...
    const getPengjuanDitolak = async () => {
        const resp = await DashboardAdminRepository("user", EnumTipeDashboard.tolak)
        if (resp !== null) {
            setDashboard((prevState) => {
                return {
                    ...prevState,
