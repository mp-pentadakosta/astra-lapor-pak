...
import { RepositoryHistoryAtasan } from "@/repository/atasan/history/RepositoryHistory";


export const HistoryAtasanViewModel = () => {
    ...
    const getHistory = async ( tglMulai : string, tglSelesai : string ) => {
        ...
        setLoading( true )
        const resp = await RepositoryHistoryAtasan( tglMulai, tglSelesai )
        if ( resp !== null ) {
